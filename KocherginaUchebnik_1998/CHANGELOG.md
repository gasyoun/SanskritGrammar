...
- **Четыре правки оцифровки внесены в `errata.yml` как `kind: digitization` (H1804, Opus 5 1M
  ...
  лежало списком — только прозой в changelog и handoff'ах. Теперь все четыре (занятие 30 — H1790;
  занятия 5 и 7, три маркера — H1795) стоят строками в
  [`ERRATA.mdx`](ERRATA.mdx) с колонками **Read / Instead of** (`II.` вместо `II` и т. д.),
  **Kind** = ⚙ digitization, **Line / locus** (строка `.mdx` + `w14:paraId` в `.docx`) и
  **Status** = ✓ fixed in 0.16.0 / 0.17.0 с датой. Печатные опечатки книги по-прежнему **не**
  заведены и заводиться из `.mdx` не будут — guardrail «не выдумывать errata» не тронут: строка
  `digitization` фиксирует, что оцифровка разошлась **сама с собой**, и ничего не говорит о том,
  что стоит в бумажном издании 1998 года. Именно ради этой границы и добавлено поле `kind`.
...
- **Занятия 5 и 7: точки у трёх оставшихся маркеров упражнений (H1795, Opus 5 1M
  ...
  аудит, что второго такого маркера в книге нет. Утверждение было **неверным**: тот аудит
  искал только маркеры, стоящие *одни на строке*, и потому не видел маркеров без точки,
  за которыми на той же строке идёт текст. Сплошная проверка всех 178 маркеров нашла ещё
  три — занятие 5 (`II Напишите шрифтом devanāgarī:`), занятие 7 (`III Напишите шрифтом
  devanāgarī и переведите слова:` и `IV Прочтите текст…`). Все три расходились со своими
  же соседями по занятию, у которых точка стоит. Исправлено **в источнике**
  (`Kochergina_unicode.docx`, по одному символу на маркер); архив проверен поэлементно:
  28/28 записей на месте и в прежнем порядке, отличается ровно одна — `word/document.xml`,
  ровно на **+3 символа**, CRC цел. В сгенерированный `.mdx` перенесены ровно три строки —
  байт в байт то, что выдала настоящая перегенерация (её прогоняли и откатывали ради
  этого); полная перегенерация по-прежнему не делается, потому что тянет ~84 посторонние
  строки и трогает медиа в `GasunsDhatu_2014`. **Повторный сплошной аудит: 0 маркеров без
  точки из 178.** Заодно в
  [`EXERCISE_BLOCK_COVERAGE_KOCHERGINA_2026.md`](EXERCISE_BLOCK_COVERAGE_KOCHERGINA_2026.md)
  форма «без точки» убрана из перечня (её больше нет), а перечень получил точные счётчики:
  обычная 101, экранированная точка 67, без пробела 5, полужирная 4, маркер один на
  строке 1 — и замер, что наивное `^[IVX]+\.` ловит 107 из 178 и пропускает 71.
  Числа занятий в таблице покрытия не изменились.
...
- **Занятие 30: маркеру упражнения II вернули точку (H1790, Opus 5 1M
  ...
  голое `II` — единственный во всей книге маркер задания **без точки**, при том что его
  соседи III/IV/V в том же занятии набраны как `III. …`. Из-за этого нумерация заданий
  занятия 30 читалась машиной как I, III, IV, V. Правка внесена **в источник**
  (`Kochergina_unicode.docx`, один символ в одном прогоне абзаца); проверено
  поэлементным сравнением архива: 28/28 записей на месте и в прежнем порядке,
  отличается ровно одна — `word/document.xml`, ровно на один добавленный символ,
  CRC цел. Уменьшение файла на 67 КБ — следствие пересжатия архива, не потери
  содержимого. В сгенерированный `Kochergina_unicode.mdx` перенесена ровно одна строка
  (`II` → `II\.`) — байт в байт то, что выдала настоящая перегенерация.
  **Почему не полная перегенерация:** `npm run convert` + `site_tools.py images` на
  текущем Pandoc 3.9.0.2 меняет в этом `.mdx` ещё ~84 посторонние строки (переэкранирование
  хвостовых табличных тегов, добавление `slug:` во frontmatter) и трогает медиа в
  GasunsDhatu_2014 — то есть закоммиченный `.mdx` старше своего пайплайна. Приведение
  его к пайплайну целиком оставлено отдельной осознанной работой.
  Аудит всей книги: маркеров, стоящих одни на строке, ровно два — занятие 22 (стр. 5459,
  точка была на месте) и занятие 30 (исправлено); второго пропущенного маркера нет.
  [EXERCISE_BLOCK_COVERAGE_KOCHERGINA_2026.md](EXERCISE_BLOCK_COVERAGE_KOCHERGINA_2026.md)
  пересчитан: занятия 22 и 30 теперь 5 (I–V), разрывов нумерации не осталось ни в одном
  из 35 занятий с блоком.
