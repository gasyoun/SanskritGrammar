...
(готовые локализованные кандидаты — с 15-07-2026 занесены в errata.yml и печатный лист) и инструменты для 7 UNTESTABLE (семантическая разметка,
...
> **HB-268 (пропорции двигу, Урок XXXIII) — блокер измерен, а не предположен (16-07-2026, H1008):** структурный класс «числительное-первый член» находится тэггером H1004, но перепись рода конечных членов (5 763 кластера, покрытие 99,9 %) дает **56 % мужского рода** — класс завален числительными БАХУВРИХИ (тип caturbhuja), которые двигу не являются; без функциональной разметки (зонд OCH-60) пропорции «большей частью / реже» непересчитываемы. Тупик записан с числами — [negативный пилот](https://github.com/gasyoun/SanskritGrammar/blob/main/ZalizniakOcherk_1978/och58_compound_stats.json).
