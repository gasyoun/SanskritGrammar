...
## [0.4.0] - 2026-07-20
### Added
- **Scherzl's 1 168 case-government relations adjudicated against the DCS treebank (H1372, Opus 4.8 `claude-opus-4-8`).** Every governed-case relation in [`government_lexicon.jsonl`](https://github.com/gasyoun/SanskritGrammar/blob/main/BuhlerLeitfaden_1923/government_class_index/government_lexicon.jsonl) is checked, root by root, against the [dcs-conllu](https://github.com/gasyoun/dcs-conllu) dependency treebank (15 900 CoNLL-U files, 5.69 M tokens, 754 726 sentences) — new [`SCHERZL_GOVERNMENT_CORPUS_ADJUDICATION_2026.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/BuhlerLeitfaden_1923/government_class_index/SCHERZL_GOVERNMENT_CORPUS_ADJUDICATION_2026.md) report, [`government_corpus_verdicts.tsv`](https://github.com/gasyoun/SanskritGrammar/blob/main/BuhlerLeitfaden_1923/government_class_index/government_corpus_verdicts.tsv) + richer [`government_vs_dcs_adjudication.jsonl`](https://github.com/gasyoun/SanskritGrammar/blob/main/BuhlerLeitfaden_1923/government_class_index/government_vs_dcs_adjudication.jsonl) (per-relation verdict + evidence), [`dcs_verb_government_profiles.json`](https://github.com/gasyoun/SanskritGrammar/blob/main/BuhlerLeitfaden_1923/government_class_index/dcs_verb_government_profiles.json) (corpus valency frame per verb), and the [`adjudicate_government_vs_dcs.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/BuhlerLeitfaden_1923/government_class_index/adjudicate_government_vs_dcs.py) + [`aggregate_dcs_gov.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/BuhlerLeitfaden_1923/government_class_index/aggregate_dcs_gov.py) builders.
  - **Result: Scherzl's 19th-century government catalogue is strongly corroborated.** Four pre-registered verdict tiers on two strictly-separated evidence layers — a direct dependency **government arc** (case-marked child of the verb; strong) and **co-occurrence measured against chance** (expected = verb_freq · corpus base-rate of the case). Verdicts: **693 CONFIRMED (59.3 %)**, **1 CONTRADICTED (0.1 %)**, 288 UNATTESTED-INSUFFICIENT (24.7 %), 186 NOT-ADJUDICABLE (15.9 %). On the **982 adjudicable** relations, **70.6 % are confirmed by a direct arc and just one is contradicted**.
  - **The one contradiction hand-adjudicates to no defect.** √arc + ablative is an `example_only` reading Scherzl himself marks `replaceable_by: instr`; the corpus attests exactly that instrumental alternant (arc governs Acc·14/Dat·9/**Ins·3**, no ablative) — system-vs-usage divergence, not a transcription error, so **zero errata routed**. The CONTRADICTED double gate (well-observed verb **and** below-half-chance co-occurrence) is deliberately conservative so a sparse parse cannot manufacture a contradiction — a case that co-occurs heavily (jñā + genitive) is never flagged just because the ~4 %-parsed treebank missed the arc.
