...
## Целевое использование / известное неправильное использование

- **Целевое использование.** Внутренний рабочий справочник для МГ; источник кандидатов
  сносок для автора Талмуда (И. Толчельников) через
  ...
  как уже выполненное сравнение (findings, не открытый вопрос).
- **Неправильное использование.** (1) Цитировать записку вовне как опубликованный/рецензируемый
  результат — это не статья, публикация не решена (см. «Назначение и аудитория» выше).
  (2) Опираться на статусы FN-0015…FN-0017 из раздела 6 subject-документа как «все pending» —
  на 09-07-2026 FN-0015 закрыт `rejected`, FN-0016 закрыт `approved`, только FN-0017 остаётся
  открытым; см. «Статус устаревания» ниже — этот метадок сам не поспевал за изменением. (3)
  Использовать инвертированную нумерацию позиций Талмуда (1=вриддхи…3=слабая) как эквивалент
  нумерации 1975 г. (1=нуль…3=вриддхи) без явного пересчёта — находка №2 существует именно
  потому, что смешение этих нумераций даёт неверные выводы.

## Поддержка и план завершения (sunset)

- **Кто поддерживает.** МГ / любая сессия, продолжающая линию H357 при появлении новых данных
  (ответ автора по FN-0017, новые покорневые расхождения, полный проход по Таблице 4 1975 г.
  — backlog-пункт 1).
- **Триггер обновления.** Смена статуса любой сноски FN-00xx в
  [`footnote-proposals/proposals.yml`](https://github.com/gasyoun/SanskritGrammar/blob/main/TolchelnikovTalmud_2026/footnote-proposals/proposals.yml);
  расширение покорневой выборки (backlog-пункт 1); формальный тип-мэппинг (backlog-пункт 2).
- **Sunset-условие.** Записка завершает свою функцию при (а) закрытии FN-0017 и снятии
  backlog-пунктов 1–2, после чего она переходит в чисто архивный статус, либо (б) решении
  МГ публиковать материал как статью (backlog-пункт 4) — тогда она конвертируется в `.mdx`
  и данный `.md`+`.meta.md` пара архивируется в пользу сайтовой версии.
