...
## [0.34.0] - 2026-07-16
### Changed
- **A65 advanced 3/5 → 4/5: полный текст статьи со слиянием A60 + враждебный рефери-проход (H1015)** — [the manuscript](https://github.com/gasyoun/SanskritGrammar/blob/main/REPORT_GRAMMAR_CLAIM_VERIFICATION_SYNTHESIS_2026.md) is now a full article (введение/обзор/метод/результаты×3/обсуждение/ограничения/заключение/приложения/литература, RU per the venue ruling); §5 absorbs A60's central divergence table + four-way typology (all 12 full-book Kochergina divergences were in the original core — the typology covers the book entirely). A fresh-context hostile referee found **10 Major + 16 Minor**; applied same pass ([disposition](https://github.com/gasyoun/SanskritGrammar/blob/main/REVIEW_A65_SYNTHESIS_REFEREE_2026.md)) — highlights: the §9 «96–100%» arithmetic corrected to 94.6–100% among testable; HK-16 restored as the programme's chronologically first substantive FALSE; the typology no longer silently extrapolated (Бюлер's 7 OVERSTATED characterized); two-axis counts per book; Fisher exact p = 0.42 inserted; per-genre word-order breakdowns inlined; systematic de-jargonization. Deferred to author-pass: banner, Zenodo deposit, GOST, Russian-scholarship ring. (Fable 5 `claude-fable-5`, [H1015](https://github.com/gasyoun/Uprava/blob/main/handoffs/H1015-Fable_SanskritGrammar_h797-programme-synthesis-report-ru_16.07.26.md))
