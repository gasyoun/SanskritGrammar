...
## [Unreleased]
### Added
- **Zaliznyak-made-learnable on-ramp (pedagogy W1c)** — a graded, minimal-notation front door into the Ряд → Тип → seṭ apparatus that sits *before* the Толчельников *Талмуд* (progressive disclosure). Four `.mdx` pages under [`TolchelnikovTalmud_2026/onramp/`](https://github.com/gasyoun/SanskritGrammar/tree/main/TolchelnikovTalmud_2026/onramp) grouped in their own sidebar sub-category — the "three questions" (какая ступень / слушается ли корень / нужна ли `i`) distilled from [`student-roadmap.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/TolchelnikovTalmud_2026/student-roadmap.md)'s Master Pipeline — reusing the existing zero-prop `AblautMachine`/`SetTree` widgets and `ZRef` deep-links (no new widget code). Design of record: [`ZALIZNYAK_ONRAMP_DESIGN.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/TolchelnikovTalmud_2026/ZALIZNYAK_ONRAMP_DESIGN.md) (+ `.meta.md`). `npm run build` green, 0 new broken links; every "one tap deeper" link resolves to its Талмуд chapter / Zaliznyak §. Wave-1c of the digital-pedagogy field (H915). (Opus 4.8 `claude-opus-4-8[1m]`)
