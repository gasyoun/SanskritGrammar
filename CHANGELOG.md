...
- H4484 (OxAlpha, glm-5.3-flash): Bibliotheca Sanscritica series onboarded —
  27-tom state census (14 издан / 2 в правке / 11 не начат, evidence per tom,
  @DECIDE memo reprint/e-publish/new-tom) in
  [BibliothecaSanscritica/BIBLIOTHECA_SANSCRITICA_CENSUS_2026-09-10.md](https://github.com/gasyoun/SanskritGrammar/blob/main/BibliothecaSanscritica/BIBLIOTHECA_SANSCRITICA_CENSUS_2026-09-10.md);
  ...
  paragraphs, 76k Devanagari chars; 92.9% lexical coverage vs 09.2014 control
  versta) in
  [BibliothecaSanscritica/LihushinaChrestomathy_2015/](https://github.com/gasyoun/SanskritGrammar/tree/main/BibliothecaSanscritica/LihushinaChrestomathy_2015)
  via the committed extractor
...
- **H1913 Slice C0 — SG-MO-021 future article pre-cutover invariants frozen.**
  ...
  + machine-readable
  [`h1913_c0_invariants.json`](https://github.com/gasyoun/SanskritGrammar/blob/main/docs/architecture/baseline/h1913_c0_invariants.json)
  regenerated by
  [`scripts/sg_mo_021_c0_freeze.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/scripts/sg_mo_021_c0_freeze.py)
  (determinism verified — two runs byte-identical): sha256 (raw + LF-normalized)
  for all 5 bounded-context files, stable IDs (`art:future`, 14 example ids),
  the full published-numbers block (future 21 556 of 523 721 finite = 4,12 %
  [4,06–4,17]; simple 20 216 / periphrastic 1340; conditional 340; participle
  1575; person/number/mood + top forms), seed 20260717 / sample 50 / universe
  predicates, DCS snapshot pin `04e0778` + sqlite sha256, article route + 15
  anchors, ledger state, 5 code-level consumers, issue-563 gate clean.
  `article_validate.py --all` PASS at the frozen commit. **C1–C4 blocked** on the
  same Slice A remainder as H1912 B1–B4 (H1911 was registry-closed against the
  A3-only [PR #569](https://github.com/gasyoun/SanskritGrammar/pull/569)).
  (Fable 5 `claude-fable-5`, human override of the Codex model lock on H1913)
...
- **H1912 Slice B0 — KnauerFrazy_1908 pre-cutover invariants frozen.**
  ...
  + machine-readable
  [`h1912_b0_invariants.json`](https://github.com/gasyoun/SanskritGrammar/blob/main/docs/architecture/baseline/h1912_b0_invariants.json)
  regenerated by
  [`scripts/knauer_b0_freeze.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/scripts/knauer_b0_freeze.py):
  sha256 (raw + LF-normalized) for all 11 tracked bounded-context files, both site
  routes + 20 anchors, errata 30 raw → 25 merged entries, parse audit KN-1..KN-214
  (210 CONFIRMED · 4 QUESTIONABLE), release provenance (book 0.3.0 / repo 0.119.0),
  7 code-level consumers. **B1–B4 blocked:** the Slice A shared contracts
  (`sg_tooling`, pipeline schema, `sg` CLI) are absent from `origin/main` —
  ...
  was registry-closed against the A3-only WIP-recovery
  [PR #569](https://github.com/gasyoun/SanskritGrammar/pull/569); the remainder is
  re-minted as a fresh handoff in Uprava. (Fable 5 `claude-fable-5`, human override
  of the Codex model lock on H1912)
...
- **Errata теперь различают правку печати и правку оцифровки — поле `kind` (H1804, Opus 5 1M
  `claude-opus-5[1m]`, 28-07-2026).** До сих пор `errata.yml` умел описывать только опечатки
  **печатного** издания, и его же собственный guardrail запрещал заводить строки из `.mdx`
  («оцифровка не позволяет отличить опечатку набора 1998 от артефакта конвертации»). Из-за
  этого правки самой оцифровки нигде не собирались в список — они жили прозой в changelog и
  handoff'ах. Теперь у записи есть **`kind`: `print` (по умолчанию) | `digitization`** —
  ортогонально `tier` (ACL-модель отвечает на «насколько сильная правка», `kind` — на «что
  правим»), с отдельной колонкой в таблице, счётчиком в шапке и явной оговоркой, что строка
  `digitization` **ничего не утверждает о бумажной книге**. Спутник — **`locus`** (файл+строка
  `.mdx` и `w14:paraId` в `.docx`): у таких строк нет печатной страницы, поэтому позиция
  показывается в колонке `Line / locus` и входит в ключ дедупликации и в checksum — без этого
  две правки в разных местах с одинаковой парой read/instead **схлопывались в одну строку**
  (поймано на живых данных: занятия 30 и 5, обе `II` → `II.`). Обратная совместимость проверена:
  `locus` попадает в payload checksum **только когда задан**, поэтому checksum ни одной уже
  опубликованной печатной строки не изменился (сверено по диффу всех восьми книг — все
  контрольные суммы парные); id строк без страницы получают сегмент `.dig.` вместо пустого
  (`…-1998.dig.1`, а не `…-1998..1`). Тесты: 167 passed, из них четыре новых на дедупликацию
  по `kind`/`locus` и на неизменность checksum без `locus`.
...
- **H1674 — methodichka visa-apply runbook (Sonnet 5 `claude-sonnet-5`, 27-07-2026).**
  ...
  **re-sheet** (the note is a research question, not an edit — moved to
  ...
  [H1674](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1674-Sonnet_SanskritGrammar_runbook-methodichka-visa-apply_26.07.26.md).
...
- **H1514 — three-tier ACL-style errata schema + stable sentence-id grammar (Sonnet 5 `claude-sonnet-5`).** [`scripts/build_errata.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/scripts/build_errata.py) generalizes to the [ACL corrections model](https://aclanthology.org/info/corrections/): every entry gets a `tier` (**erratum** default / **revision**, id gains `.v2` via `revises: <id>` / **retraction**, watermarked + needs a `reason`, never deleted), a stable `id` (`<book-slug>-<edition-year>.<page>.<n>`), a `date`, and a content `checksum`. Per-book output moved from `ERRATA.md` to `ERRATA.mdx` so it renders live on the Docusaurus site via the existing auto-discovery (zero `docusaurus.config.mjs` edits) — all 8 books' errata pages now build. [`TolchelnikovTalmud_2026/errata.yml`](https://github.com/gasyoun/SanskritGrammar/blob/main/TolchelnikovTalmud_2026/errata.yml) carries one labelled schema-demonstration example of each tier (no book in this corpus has yet had a genuine revision or retraction). Separately, [`scripts/data/sentences.json`](https://github.com/gasyoun/SanskritGrammar/blob/main/scripts/data/sentences.json)'s 3,213 exercise-sentence ids moved from `buhler-I-1` to the `YEAR.VOLUME.NUMBER`-style `buhler-1923.I.1` (roadmap B2, [ACL id policy](https://aclanthology.org/info/ids/)); every downstream artifact that embeds these ids — `matches.json`, the hand-labeled gold set [`matches_review.tsv`](https://github.com/gasyoun/SanskritGrammar/blob/main/scripts/data/matches_review.tsv) (128 human verdicts preserved, only the id columns remapped), `buhler_provenance.{json,csv}`, `fidelity.json`, and prose citations in `BUHLER_SENTENCE_PROVENANCE_ADJUDICATION.md`/the Kochergina методичка — was migrated in lockstep (content verified byte-identical except the id format). `pytest` 195/195, `npm run build` green (per-book errata pages present, all three tiers render). Roadmap [Q3.5 / Angle B1–B2](https://github.com/gasyoun/SanskritGrammar/blob/main/ROADMAP_GRAMMAR_CORPUS_ACL_2026_2027.md). Handoff [H1514](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1514-Sonnet_SanskritGrammar_errata-corrections-schema-and-ids_22.07.26.md).

## [0.114.0] - 2026-07-27
### Added

- **§4e′ — the PM bars now carry their provenance and an expiry date (Opus 5 1M `claude-opus-5[1m]`, 27-07-2026).** [`DIGITAL_SANSKRIT_PEDAGOGY_FIELD_2026.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/DIGITAL_SANSKRIT_PEDAGOGY_FIELD_2026.md) §4e′ sets a **27-09-2026 review point** and records, per bar, *what it was argued from*, how strong that anchor is, and *what evidence would move it* — so recalibrating in two months is reading a table, not re-deriving twelve thresholds. The honest split it makes visible: **4 of 12 bars rest on a measurement** (PM2 on the 90.7% keyed share; PM3 on the 30-of-top-100 function-word count; PM4 between the measured τ ≈ 0.05 frequency floor and the 0.446 weakest textbook pair; PM7 trivially, from a zero baseline), one is a disclosure rule with no threshold (PM6), and **7 are reasoned but unanchored** (PM1, PM5, PM8, PM9, PM10, PM11, PM12). **PM8 and PM12 are computable from existing data** — the cheapest pre-review improvement, needing no new build. Revision protocol fixed in the same section: a bar changes in the open, with the measurement or argument that moved it and a metadoc history row keeping the superseded value, and **never in the same pass as the measurement that failed it**. Mirrored as a `@DECIDE` row in [`Uprava/GTD_NEXT_ACTIONS.md`](https://github.com/gasyoun/Uprava/blob/main/GTD_NEXT_ACTIONS.md) so the review point surfaces to a human on time rather than depending on someone re-reading the doc. Follow-up to [H1476](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1476-Opus_SanskritGrammar_pedagogy-aspect-measurable-result-metrics_22.07.26.md).

## [0.113.0] - 2026-07-27
### Added

- **H1476 — a falsifiable "first measurable result" metric for each of the 12 pedagogy aspects (Opus 5 1M `claude-opus-5[1m]`, 27-07-2026).** [`DIGITAL_SANSKRIT_PEDAGOGY_FIELD_2026.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/DIGITAL_SANSKRIT_PEDAGOGY_FIELD_2026.md) §4e now carries the **PM1–PM12** register — one metric per aspect, each with its denominator, data source, today's value, progress bar and **refutation condition** (a metric with no refutation condition is not admitted). Each PM targets its aspect's own stated gap, so "the metric moved" and "the gap closed" are one sentence. The register is a **capability** layer by design: a companion table fixes how the PMs feed RQ1–RQ3 and bars any PM from measuring learning gain or retention — [RQ4's protocol](https://github.com/gasyoun/SanskritGrammar/blob/main/docs/RQ4_EVALUATION_PROTOCOL_2026.md) keeps one outcome instrument for the whole field, so twelve aspects cannot grow twelve incompatible rulers. Two baselines are measured rather than asserted: **PM2 = 90.7%** answer-keyed drill items (40 863 of 45 045) and **PM6 = 56.5% by type / 56.2% by DCS token mass** for the Zaliznyak on-ramp's 4 taught ablaut rows — the near-identical pair showing the rows are frequency-neutral, so the on-ramp buys scope proportional to its size rather than a high-frequency shortcut. New [`TolchelnikovTalmud_2026/tools/measure_onramp_scope.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/TolchelnikovTalmud_2026/tools/measure_onramp_scope.py) reproduces PM6. Ten of twelve PMs read "unmeasured" — the honest state of the field, recorded as such. Metric column added to the §4a matrix and the §3.8–3.12 table; §8 makes a PM mandatory for any new aspect. Handoff [H1476](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1476-Opus_SanskritGrammar_pedagogy-aspect-measurable-result-metrics_22.07.26.md).

## [0.112.0] - 2026-07-26
### Fixed

- **[integrity] PWG compound layer re-derived headword-anchored + bracket-aware ([#527](https://github.com/gasyoun/SanskritGrammar/issues/527); Opus 5 1M `claude-opus-5[1m]`, 26-07-2026).** [`scripts/pwg_compound_split.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/scripts/pwg_compound_split.py) took the first `+`-chain anywhere in the first 400 chars of an entry, so 344/16,738 rows of a dataset advertised as **high-precision splitter gold** shipped a bracketed *inner* sub-analysis (`akṛttaruc` → `a + kṛtta`, true `akṛtta + ruc`) or a *neighbouring* word's parenthesis (`adhikaṣāṣṭika` → `adhika + ṣaṣṭi`, which composes a different headword). Now anchored on the entry's own `{#headword#}¦`, `[...]`-masked, split at paren depth 0, one member per `+`-part, with multi-candidate parts settled against the headword's surface (`von {#BAnumant#} oder von {#BAnu#} + {#mati#}` → `BAnu + mati`) and genuine ambiguity **dropped, never guessed**, counted by reason in [`pwg_compound_summary.json`](https://github.com/gasyoun/SanskritGrammar/blob/main/data/pwg_compound_split/pwg_compound_summary.json). [`data/pwg_compound_split/pwg_compound_splits.tsv`](https://github.com/gasyoun/SanskritGrammar/blob/main/data/pwg_compound_split/pwg_compound_splits.tsv) regenerated: **17,112 rows** (was 16,745) — 16,094 unchanged · 139 members corrected · 512 dropped as unresolvable · 879 added that the old head-scan missed. 17 fixtures via `--selftest`. `pwg_toplevel()` is kept in sync with the same function in SanskritLexicography's `adjudicate_compound_differs.py`, which adjudicates this layer against Monier-Williams. Consumers named in the dataset README (kosha DCS-sandhi programme, SanskritSpellCheck splitters) should re-pull. Handoff [H1703](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1703-Opus_SanskritLexicography_compound-differs-second-arm-and-sheet-binding_26.07.26.md).
...
### Added
- **Grammar-claim / kāraka encoding-methodology manual (Wave 3, H1406; Fable 5 `claude-fable-5`, 25-07-2026).** New normative method doc [`sangram/SANGRAM_GRAMMAR_CLAIM_ENCODING_METHOD.mdx`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/SANGRAM_GRAMMAR_CLAIM_ENCODING_METHOD.mdx) (RU body + EN abstract, slug `/sangram/grammar-claim-encoding-method`, sibling of contract C3, no manifest/toc_ref) + companion [`sangram/SANGRAM_GRAMMAR_CLAIM_ENCODING_METHOD.meta.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/SANGRAM_GRAMMAR_CLAIM_ENCODING_METHOD.meta.md) — the repo's **first metadoc carrying the `LAST_VERIFIED` staleness block** (joins the org `manual_staleness.py` estate). Encodes as reusable rules: falsifiable-claim criterion + `claims.yml` register format; fact × presentation verdict axes with the D-B source-priority rule; the four-way divergence typology as a top-down classifier with two testable consequences; the probe → verify adversarial workflow; the kāraka↔UD-deprel proxy table with its three systematic non-mappings (passive kartṛ→`obl:agent`→Ins; sampradāna three-way split; `nsubj:pass`=0 range rule); etalon-layer rules (Usha Sanka corpus **published 2014, reuse with scholarly attribution**; H1399 per-lemma agreement recorded as merged [PR #496](https://github.com/gasyoun/SanskritGrammar/pull/496)); reproducibility pitfalls (denominator universes, kappa rederivation, pin-by-tag after the 15-07-2026 c3-pin recovery, local-only scripts vs CI). Authoring-pass verification: `rederive_kappa.py` reproduced κ 0.929/0.720 exactly; `check_denominator_commensurability.py` OK (34 summaries); `pytest` green; `npm run build` green; Usha "unpublished" grep over origin/main = **0 live stale surfaces** (9 hits all corrected-historical or unrelated). Handoff [H1406](https://github.com/gasyoun/Uprava/blob/main/handoffs/H1406-Fable_SanskritGrammar_deep-manual-karaka-claims-methodology-wave3_20.07.26.md), per the org deep-manuals programme ([PLAN](https://github.com/gasyoun/Uprava/blob/main/docs/PLAN_ORG_DEEP_MANUALS_FABLE_WAVES_2026H2.md), Uprava-private).
- **`/ask` plan — pedagogy last-mile residual (Grok 4.5 `grok-4.5`, 25-07-2026).** Layered plan set: cover [`docs/PLAN_SANSKRITGRAMMAR_PEDAGOGY_LAST_MILE_2026H2.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/docs/PLAN_SANSKRITGRAMMAR_PEDAGOGY_LAST_MILE_2026H2.md) (+ [`.meta.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/docs/PLAN_SANSKRITGRAMMAR_PEDAGOGY_LAST_MILE_2026H2.meta.md)) · [roadmap](https://github.com/gasyoun/SanskritGrammar/blob/main/docs/ROADMAP_SANSKRITGRAMMAR_PEDAGOGY_LAST_MILE_2026H2.md) · [architecture](https://github.com/gasyoun/SanskritGrammar/blob/main/docs/ARCHITECTURE_SANSKRITGRAMMAR_PEDAGOGY_LAST_MILE.md) · [implementation](https://github.com/gasyoun/SanskritGrammar/blob/main/docs/IMPLEMENTATION_SANSKRITGRAMMAR_PEDAGOGY_LAST_MILE.md) · [verification](https://github.com/gasyoun/SanskritGrammar/blob/main/docs/VERIFICATION_SANSKRITGRAMMAR_PEDAGOGY_LAST_MILE.md). 30 interview rulings; autonomy gate **PASS**. Wave-1: reuse methodichka H1454+H1615 · mint **H1643** export adapter · **H1644** Systema hop smoke. Distinct from freeze-exit plan (H1611–H1615); no prod RQ4 flip.
- **H1614 — SE freeze-survivor visa sheet disposition: zero survivors → no sheet (Grok 4.5 `grok-4.5`, 24-07-2026).** Upstream [H1612](https://github.com/gasyoun/SanskritGrammar/pull/517) left **0** clear-gate SE survivors (1 kill_gated · 7 blocking_note). Per the W1-A3 stop condition, closed without `review/specs/sangram-se-freeze-survivors-visa_*.json`, without `build_visa_sheet.py` HTML, and without a [REVIEW_SHEETS_INDEX](https://github.com/gasyoun/Uprava/blob/main/REVIEW_SHEETS_INDEX.md) row. Disposition note: [`sangram/audit/H1614_se_freeze_survivors_visa_disposition.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/audit/H1614_se_freeze_survivors_visa_disposition.md). Fence: no votes invented; no kill_gated cards; no `freeze.active` flip.
- **H1613 — MO/WF freeze-exit probes (park-and-skip; Grok 4.5 `grok-4.5`, 24-07-2026).** All **7** MO/WF `disposition=unknown` baseline stragglers (SG-MO-019/022/024/025, SG-WF-002/009/011) lack a named C5 § 7 pilot kill-gate per the H1611 matrix — no invented thresholds. Per-article artifacts `sangram/audit/probe_freeze_<slug>.{py,json,md}` + rollup [`probe_freeze_mo_wf_rollup_H1613.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/audit/probe_freeze_mo_wf_rollup_H1613.md). Ledger: leave `unknown`, set `blocking_note` + `source_links`. Outcomes: **0** kill_gated · **0** clear-gate survivors · **7** escalated (visa-eligible non-SE list for a later sheet, not H1614). W1-A2 of the freeze-exit plan.
- **H1611 — freeze-exit kill-gate matrix for 15 unknown baseline candidates (Grok 4.5 `grok-4.5`, 24-07-2026).** New [`sangram/audit/FREEZE_EXIT_KILLGATE_MATRIX_2026.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/audit/FREEZE_EXIT_KILLGATE_MATRIX_2026.md): one row per `disposition=unknown` ledger id → C5/C6 programme slot → quoted pilot kill-gate or **`MISSING`** → existing `scripts/sg_*` path → acceptance/escalate. Result: **1/15** has a fireable named gate (SG-SE-006 = C6 § 7 P2); **14/15** criterion-`MISSING` with escalate paths; **15/15** scripts present. No probes run. W1-A0 of the freeze-exit plan; feeds H1612/H1613.
- **H1612 — SE freeze-exit probes (W1-A1): 8/8 unknown SE baselines instrumented; SG-SE-006 `kill_gated`; zero SE survivors for H1614 (Grok 4.5 `grok-4.5`, 24-07-2026).** Per-article artifacts `sangram/audit/probe_freeze_<slug>.{py,json,md}` + runner [`probe_freeze_se_cluster_h1612.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/audit/probe_freeze_se_cluster_h1612.py). **SG-SE-006** (C6 § 7 P2): native tags assign a classical preterite on **42.22%** of finite past (62 795 / 148 750) ≪ 95% → disposition **`kill_gated`**. Seven non-pilot SE slots (SE-001/002/003/004/005/008/013) have **no § 7 kill-gate** — integrity re-derive vs pin `04e0778` all PASS; ledger `blocking_note` (no invented thresholds). Survivor roll-up: [`probe_freeze_se_H1612_survivors.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/audit/probe_freeze_se_H1612_survivors.md) (empty list → H1614 closes without a visa sheet). Fence: no `freeze.active` flip. `article_validate --all` PASS · `pytest` green.
...
### Changed
- **H1274 — W1-C remainder: the five-sheet editorial-note backlog terminalised, measured scope 14 open (not the plan's 22) (Fable 5 `claude-fable-5`).** Re-adjudication against `origin/main` confirmed the [handoff's](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1274-Fable_SanskritGrammar_sangram-apply-notes-articles-remainder_18.07.26.md) own suspicion: taddhita `WF004-05/-08/-10` were applied by H1183 on 17-07-2026 and `WF004-06`'s substance by H1189/H1192/H1254 (divergence reported, plan table untouched, gate B5), on top of the perfect/prose divergence [H1273](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1273-Sonnet_SanskritGrammar_sangram-review-votes-track-h856-reversal_18.07.26.md) had already recorded. The 14 genuinely open notes went terminal: **applied** — [declension-overview](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/articles/declension-overview/index.mdx) A2 (Сложные слова + местоимения/прилагательные), A3 (зонд редкости дательного: «почти не засвидетельствован» подтверждён на уровне лемм 8,5 % и в Dat.Du 576 токенов, отклонён для падежа — 235/270 текстов; 2,2 % не тронуто), A7 (§ 5 п. 4 по-русски), B1 (§ 1.1 сверка с сеткой «Языков мира» Елизаренковой: 7 граммем против 8 строк разметки), [perfect](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/articles/perfect/index.mdx) A5 (лексиконный зонд: 39 375 финитных токенов 59 заведомых перфектов, полнота форма-класса **0 %**); **policy → style guide only (C5)** — [§ 3.7 «не меньше пяти примеров»](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/editorial/SANGRAM_STYLE_GUIDE_PROSE_RU.mdx) (declension#A5 + WF004-09) и § 4 признак 7 «запрет самодельных терминов» (пятилистовая семья), § 1 RWS-council standing rule (prose#A1), § 1.1 жизненный цикл по Chicago Manual (prose#B1 + perfect#B1), четыре git-history антипримера (prose#A2, первая половина); **answered** — A6/TAD2-08 (страницы живы на githubio; таддхита-URL в шапке чинился — не хватало `grammars/`, было 404) и WF004-05 (Whitney §§ 1210–1230 уже в § 5); **deferred** — WF004-06 residue ([probe stub](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/audit/PROBE_STUB_WF004-06_DICT_TADDHITA_APTE_SKD_VCP.md): csl-orig skd/vcp/ap90 etymology TSVs are the confirmed prior art) и rusgram-половина prose#A2; **escalated** — declension#A1 (жанровый вопрос, @DECIDE в GTD). Probes: [`probe_h1274_dat_attestation.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/audit/probe_h1274_dat_attestation.py) + [`probe_h1274_perfect_lexicon.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/audit/probe_h1274_perfect_lexicon.py), published-number cross-checks reproduced (1 790 270 / 102 055 / 4 046). [`EDITORIAL_NOTE_INDEX.tsv`](https://github.com/gasyoun/SanskritGrammar/blob/main/review/EDITORIAL_NOTE_INDEX.tsv) rows updated with per-note verdicts + evidence (W1-B input). WF004-07 re-vote trigger recorded in GTD.

## [0.107.0] - 2026-07-21

### Fixed
- **Gold-provenance audit — the last outstanding refutation ran, acceptance criterion 10/10 met (H1272 close, Fable 5 `claude-fable-5`).** The [H1272](https://github.com/gasyoun/Uprava/blob/main/handoffs/H1272-Fable_SanskritGrammar_gold-provenance-contamination-audit_18.07.26.md) audit shipped 19-07 (v0.87.0) with one disclosed gap: the csl-atlas overlay-queue CONTAMINATED verdict had no adversarial refutation pass (session limit). That refutation ran 21-07-2026 and returned **CONFIRMED** — an independent recount of all 14 review-queue files on csl-atlas `origin/main` reproduced the audit's reviewer distribution exactly (130 `codex` / 7 `Antigravity` / 10 `gasyoun`; zero `reviewed-corrected`; `codex`/`Antigravity` are agent identities, not human aliases; no subsequent human gate exists; [`build-benchmark-report.mjs`](https://github.com/sanskrit-lexicon/csl-atlas/blob/main/scripts/build-benchmark-report.mjs) counts gold by `reviewStatus` alone; nothing changed post-19-07). [`GOLD_PROVENANCE_AUDIT_2026H2.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/GOLD_PROVENANCE_AUDIT_2026H2.md) census table + Limitations corrected accordingly — all ten CONTAMINATED verdicts have now survived refutation. Step-5/6 routing landed in the same pass: [SanskritLexicography FINDINGS §460](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md) (the four contamination mechanisms), per-paper blocker/caveat notes on A12/A16/A26/A29/A44/A58/A65 in [Uprava ARTICLES.md](https://github.com/gasyoun/Uprava/blob/main/ARTICLES.md), and a five-ruling `@DECIDE` repair bundle in [Uprava GTD](https://github.com/gasyoun/Uprava/blob/main/GTD_NEXT_ACTIONS.md).

## [0.106.0] - 2026-07-21

### Changed
- **H1276 — A65 verdict-validation sheet: the 9 author notes adjudicated against the H1049 fan-out, and the write-back gap closed (Fable 5 `claude-fable-5`).** The Sangram editorial-note plan carried the sheet `sanskritgrammar-a65-verdict-validation-disagreements_16.07.26` as **9 open / 0 applied**; independent re-derivation against `origin/main` confirms the true split is **7 discharged · 2 open · 8 missing write-backs** — seven notes were already discharged on 16–17-07-2026 by [H1049](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1049-Fable_SanskritGrammar_a65-adjudication-apply-9-approve_16.07.26.md)'s fan-out into five research handoffs (H1050–H1054) plus two questions-log rows, a discharge pattern that leaves no revision row for the plan's evidence ranking to see. This pass writes the merged evidence back to the point of use: each discharged entry in [`BuhlerLeitfaden_1923/claims.yml`](https://github.com/gasyoun/SanskritGrammar/blob/main/BuhlerLeitfaden_1923/claims.yml) / [`KocherginaUchebnik_1998/claims.yml`](https://github.com/gasyoun/SanskritGrammar/blob/main/KocherginaUchebnik_1998/claims.yml) now names its probe, verdict and PR — with the three against-expectation results stated unsmoothed (HK-15: в классике направление переключения перевернуто; HK-34: «весь раздел неверный» опровергнута для фактов, подтверждена для каркаса; HB-57: нейтрализация значима, но V Крамера 0,084 — содержательно ничтожен). HB-10's Whitney § 1128 caveat ruled into `mg_footnote` (the rendered apparatus; tally 23 → 24); HK-4a `ESCALATED` to a GTD `@DECIDE` (инвариантность -syá — авторская позиция, не корпусный факт; `Q2607-146` open); HK-4b cross-referenced to [H1228](https://github.com/gasyoun/Uprava/blob/main/handoffs/H1228-Sonnet_SanskritGrammar_whitney-concordance-audit_18.07.26.md), no Whitney sweep of its own. Generated files regenerated via `scripts/build_claims.py` only; per-book changelogs updated in both registers.
## [0.105.0] - 2026-07-21

### Fixed
- **Post-merge MDX build hotfix.** H1346's MO18 card left `` `feat_verbform` ∈ {NULL(=финитн.), Part, Conv, Gdv, Inf}) `` with the set notation outside its code span in [`aorist/index.mdx`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/articles/aorist/index.mdx) — MDX/acorn parses a bare top-level `{...}` as a JS expression, breaking the Docusaurus build (surfaced only on post-merge `main` CI, since neither `pytest` nor `article_validate.py --all` runs a full MDX build). Moved the set notation inside the code span, matching the safe pattern already used elsewhere in the same file and in `consonant-stems/index.mdx`. Verified `npm run build` succeeds; `main` CI green again.
...
### Added
- **SG-SE-013 `karaka-case` § 7.2 — per-lemma agreement join: Usha kāraka gold vs DCS deprel proxy (H1399, Opus 4.8 `claude-opus-4-8`).** § 7.1 compared the two models aggregate-by-role; § 7.2 answers the per-root question — for each gold dhātu DCS attests as a governing verb, does the gold's claimed kāraka-demand agree with what the corpus attests? New local script [`scripts/usha_perlemma_agreement.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/scripts/usha_perlemma_agreement.py) transliterates the gold's 581 Devanāgarī roots to IAST ([`indic_transliteration`](https://github.com/indic-transliteration/indic_transliteration_py)) and joins directly to `dcs_full.sqlite` lemmas (DCS lemmas are Dhātupāṭha roots in IAST, so the join is direct), then tallies each root's corpus-attested role set via the § 3 deprel→kāraka overlay. Committed [`data/usha_gold_perlemma_agreement.json`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/articles/karaka-case/data/usha_gold_perlemma_agreement.json), CI-gated by [`tests/test_usha_gold_perlemma_agreement.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/tests/test_usha_gold_perlemma_agreement.py) (schema/bounds over the committed table — the join needs the untracked 920 MB master, so it is not regenerated in CI). **Findings:** join coverage **212/576 (36.8 %)** roots have ≥1 kāraka-tagged corpus argument (the rest of the Dhātupāṭha tail is unattested in DCS as a governing verb — reported, not capped); **196** roots are comparable (the gold makes a core-role claim); mean Jaccard over the six core kārakas **0.52** (0.37 under a saturation-guarded salient set). kartṛ/karman the corpus confirms almost entirely; adhikaraṇa (110 gold-only), apādāna (60), karaṇa (71) the gold assigns far more freely than the corpus attests; sampradāna is corpus-heavy — the § 7.1 rank divergence (adhikaraṇa gold-heavy, sampradāna corpus-heavy) **reproduces at the per-root level**. Honest limits stated: gold granularity is the meaning-group not the root, binary corpus sets saturate for frequent verbs, coverage is capped by the 3.93 % deprel layer. No corpus figure of the existing article changed.
...
### Fixed
- **Last third-party trove work cleared — _Bhāratīya Upasargārtha-Candrikā_ (1976) rights held too (MG, Opus 4.8 `claude-opus-4-8`).** Follows 0.102.0's re-assessment: MG confirmed the 1976 *Upasargārtha-Candrikā* — the one work left "to verify" — is also **rights-held**, so **all** third-party works in the Usha trove are now cleared for reuse with attribution (Krishnacharya 1924 public-domain by age; Abhyankar 1961, Palsule 1955/1961, Upasargārtha-Candrikā 1976 rights-held by MG). Updated [`karaka-case` § 7](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/articles/karaka-case/index.mdx), the [gold README](https://github.com/gasyoun/SanskritGrammar/blob/main/Concordance/usha_karaka_gold/README.md), and the [reuse survey](https://github.com/gasyoun/SanskritGrammar/blob/main/Concordance/USHA_SANKA_KARAKA_TROVE_REUSE_SURVEY_2026.md); the "left to verify" wording in 0.102.0's entry is corrected in place. (Later same day: MG also confirmed rights to the remaining `dhatukarika.pdf` / `itviveka.pdf` scans, so **nothing in the trove is consult-only** — the whole Usha trove is cleared for reuse with attribution.)

## [0.102.0] - 2026-07-20

### Fixed
- **Usha Sanka kāraka rights caveat corrected — the corpus was published in 2014 (MG, Opus 4.8 `claude-opus-4-8`).** The extraction/wiring work (0.101.0 below) wrongly described Usha Sanka's doctoral corpus as **unpublished / internal-use / needing permission**. MG confirmed it was **published in 2014**, so her selection and analysis (the kāraka dataset, meaning-class model, role assignments) are reusable and citable with scholarly attribution, including on the public Pages site. Swept the stale caveat from [`karaka-case` § 7 + provenance](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/articles/karaka-case/index.mdx) (also deduped a doubled «Права» paragraph), the [gold README](https://github.com/gasyoun/SanskritGrammar/blob/main/Concordance/usha_karaka_gold/README.md), and the [reuse survey](https://github.com/gasyoun/SanskritGrammar/blob/main/Concordance/USHA_SANKA_KARAKA_TROVE_REUSE_SURVEY_2026.md); the parentheticals in 0.101.0's entries are corrected in place. The **third-party** trove works were also re-assessed (the blanket "reference-only" was a conservative default, not a determination): Krishnacharya *Bṛhad-Rūpāvalī* (1924) is public-domain by age; Abhyankar *Dictionary of Sanskrit Grammar* (1961) and Palsule *Dhātu Concordance* (1955/1961) are rights-held (usable with attribution); *Bhāratīya Upasargārtha-Candrikā* (1976) was flagged "to verify" here but **is also rights-held** (cleared in 0.103.0). Follow-up handoff [H1399](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H1399-Opus_SanskritGrammar_sg-se-013-karaka-perlemma-agreement-join_20.07.26.md) carries the corrected rights.

## [0.101.0] - 2026-07-20

### Added
- **Usha Sanka kāraka gold — structured dhātu → kāraka → citation dataset extracted (Opus 4.8 `claude-opus-4-8`).** [`scripts/extract_usha_karaka_gold.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/scripts/extract_usha_karaka_gold.py) parses the kāraka-demand model from the clean-Unicode `धातुकारकाकांक्षा.docx` (no OCR) into [`Concordance/usha_karaka_gold/`](https://github.com/gasyoun/SanskritGrammar/tree/main/Concordance/usha_karaka_gold) — **145 verb-groups · 581 distinct dhātus · 1,372 kāraka citations, 1,271 (93 %) with an explicit classical source** (Mahābhārata, Rāmāyaṇa, Purāṇas, Upaniṣads, Gṛhyasūtras), bucketed by 9 roles (the six Pāṇinian kārakas + tādarthya/hetu/anya). JSON (grouped) + CSV (flat) + [README](https://github.com/gasyoun/SanskritGrammar/blob/main/Concordance/usha_karaka_gold/README.md) with provenance, schema, and rights (thesis published 2014 — reuse with attribution; corrected — see `[Unreleased]`). The native, citation-backed reference layer for SG-SE-013 against the DCS `deprel` proxy. Source is untracked local bulk, so the extractor runs locally (not CI); the JSON/CSV are the shipped output.
- **Reuse survey of the Usha Sanka kāraka PhD trove (Opus 4.8 `claude-opus-4-8`).** [`Concordance/USHA_SANKA_KARAKA_TROVE_REUSE_SURVEY_2026.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/Concordance/USHA_SANKA_KARAKA_TROVE_REUSE_SURVEY_2026.md) catalogs the 361 MB untracked working corpus under `Concordance/UshaSanka_Ph.D_2014/` (Usha Sanka's 2014–15 PhD on dhātu kāraka-demand). The bound PDF's text layer is broken, but the source files are clean machine-readable data — no OCR: a structured Dhātupāṭha (`dhatu_chart.csv`, `mAdhavIya-dhAtupATha.xlsx`, `dhatu-index.xlsx`), the Aṣṭādhyāyī sūtrapāṭha (3,983 sūtras XLSX), 587 per-verb `DhatuVistara/*.html` concordance pages, `PWG-Dhatu-Upasarga-entries.htm`, and the kāraka model itself as clean Unicode `.docx` (`धातुकारकाकांक्षा.docx`, 292k Devanagari codepoints) — the native alternative to SG-SE-013's ~3.93 % `deprel` proxy. Survey tiers assets by reuse value → consumer (SG-SE-013 kāraka, GasunsDhatu_2014, pwg_ru roots, Pāṇini skills, SG-WF-011 preverbs) and flags rights (Usha Sanka's work published 2014 — reusable with attribution [corrected, see `[Unreleased]`]; third-party works cleared too — Krishnacharya 1924 public-domain, Abhyankar 1961 + Palsule 1955/61 + Bhāratīya Upasargārtha-Candrikā 1976 all rights-held).

### Changed
- **SG-SE-013 `karaka-case` wired to the Usha Sanka native kāraka gold (H1395, Opus 4.8 `claude-opus-4-8`).** The article's stated main limit is that DCS has **no native kāraka annotation** — its whole crosstab is an overlay on the ~3.93 %-coverage `deprel` proxy. New **§ 7 «Нативный слой карак: модель Уши Санки (2015)»** wires in the citation-backed reference the article was missing: the [`Concordance/usha_karaka_gold/`](https://github.com/gasyoun/SanskritGrammar/tree/main/Concordance/usha_karaka_gold) dataset (145 groups · 581 dhātus · 1,372 citations, 93 % sourced · 9 roles) as the native, lemma-level role-valency layer against which the corpus proxy can be measured — framed as two complementary layers (DCS = token distribution, broad but shallow on kāraka; Usha = lemma valency, deep on kāraka but sampled), with the rights note (thesis published 2014, reuse with attribution — corrected, see `[Unreleased]`) carried into the article's provenance. A row is added to the § 1 framework table and § 8 limit № 1 now notes the independent cross-check point exists. **§ 7.1 ships a role-level measurement** — new [`scripts/compare_usha_gold_vs_proxy.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/scripts/compare_usha_gold_vs_proxy.py) → committed [`data/usha_gold_vs_proxy.json`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/articles/karaka-case/data/usha_gold_vs_proxy.json) (CI-gated by [`tests/test_usha_gold_vs_proxy.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/tests/test_usha_gold_vs_proxy.py), both inputs committed) — with two findings: **(1)** the proxy is blind to two roles the gold attests — hetu (180) + tādarthya (33) = **213 citation-backed attestations** the UD `deprel` map has no relation for; **(2)** both models rank kartṛ/karman 1–2 but diverge mid-table — adhikaraṇa is gold-rank 3 / proxy-rank 5 (Usha's meaning-classes over-weight locus), sampradāna gold-rank 6 / proxy-rank 3 (the running corpus over-weights datival recipients). No corpus figure changed; extraction itself shipped in [#484](https://github.com/gasyoun/SanskritGrammar/pull/484). The deeper **per-lemma** agreement join (581 roots ↔ DCS deprel evidence) needs `dcs_full.sqlite` + a root↔lemma crosswalk and is left as an explicit follow-up.

## [0.100.0] - 2026-07-20

### Added
- **Universe-commensurability layer over the case cluster — the cross-article defect a number-level audit can't see (H1371, Opus 4.8 `claude-opus-4-8`).** The H1229 ledger verifies each number against its own predicate and is structurally blind to a category error across articles: `case-system-overview` reports Nom = **44.7 %** (of 3,173,636 real vibhakti, all POS) while `declension-overview` reports Nom = **38.7 %** (of 1,790,270 inflected NOUN) — both correct, incomparable. New [`sangram/audit/universe_commensurability.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/audit/universe_commensurability.py) re-derives the four case universes from the pinned master, builds the containment lattice, and adjudicates **every** cross-article case-share pair COMMENSURABLE / INCOMMENSURABLE-DECLARED / INCOMMENSURABLE-UNDECLARED — **28 pairs, 0 unclassified** (25 declared-divergent, 3 made directly comparable by the denominator fix above). Committed [`universe_commensurability_verdicts.json`](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/audit/universe_commensurability_verdicts.json) is CI-gated by [`tests/test_universe_commensurability.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/tests/test_universe_commensurability.py); the canonical-universe ruling is new method rule [П8](https://github.com/gasyoun/SanskritGrammar/blob/main/sangram/SANGRAM_CORPUS_EVIDENCE_METHOD.mdx), applied in-place to the two seed articles' §2 universe blocks (reconciliation sentence + cross-link). Write-up: [`SANGRAM_CASE_UNIVERSE_COMMENSURABILITY_2026.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/SANGRAM_CASE_UNIVERSE_COMMENSURABILITY_2026.md) (+ metadoc); ledger cross-referenced in [`DCS_DERIVED_NUMBERS_LEDGER_2026.md`](https://github.com/gasyoun/SanskritGrammar/blob/main/DCS_DERIVED_NUMBERS_LEDGER_2026.md).
...
- **Bühler provenance: third corpus, the IAST subset ruled out, and three corrections to H1212 (H1344, Opus 4.8 `claude-opus-4-8`).** [`scripts/buhler_provenance.py`](https://github.com/gasyoun/SanskritGrammar/blob/main/scripts/buhler_provenance.py) gains GRETIL plaintext as corpus C (`SanskritSpellCheck/detectors/gretil_*_raw/`, **85,401 verse lines**, +11 % haystack) behind `--no-gretil`, and a `--scripts deva,iast` mode. Headline moves 50/16/518/1 → **51 quotation · 17 adapted · 515 invented · 2 unknown** of 585.
  ...
  - Also fixed: DCS `sentence.text_sandhied` is **not reliably sandhied** (it stores analyzed word forms for `buhler-1923.XXXV.806`, which silently downgraded a verbatim Manusmṛti quotation to a partial match) — logged as a cross-repo gotcha in [FINDINGS](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md).
...
- **`verify_claims_dcs.py` (Kochergina) extended with reproducible backlog-verification metrics
  ...
  ([H797](https://github.com/gasyoun/Uprava/blob/main/handoffs/archive/H797-Fable_SanskritGrammar_claim-verification-backlog-verify-and-cross-grammar-generalise_12.07.26.md), Opus 4.8 `claude-opus-4-8`.)
