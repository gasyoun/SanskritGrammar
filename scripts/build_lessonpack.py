...
SECTION_BODIES = {
    ...
    "sparsa-varga": {
        "nol": (
            "Древнеиндийские учёные описали звуки языка очень похоже на современных "
            "фонетистов — по работе органов речи. Все шумные смычные согласные вместе с "
            "носовыми собраны в группу **sparśa** (буквально «прикосновение»: язык "
            "прикасается к нёбу).\n\nВнутри группы — **пять рядов (varga)**. В каждом ряду "
            "звуки различаются ровно по двум признакам: глухой или звонкий, без придыхания "
            "или с придыханием. Четыре «версии» одного звука плюс носовой — пять букв в "
            "ряду."
        ),
        "prodolzhayushchiy": (
    ...
    "five-rows": {
        "nol": (
            "Вот все пять рядов. Читайте по строке: сначала название ряда, потом его пять "
            "букв.\n\n"
            "1. **Заднеязычные** (произносятся глубоко, у мягкого нёба): क ka · ख kha · "
            "ग ga · घ gha · ङ ṅa\n"
            "2. **Палатальные** (язык поднимается к твёрдому нёбу): च ca · छ cha · ज ja · "
            "झ jha · ञ ña\n"
            "3. **Церебральные**, или ретрофлексные (кончик языка завёрнут назад): ट ṭa · "
            "ठ ṭha · ड ḍa · ढ ḍha · ण ṇa\n"
