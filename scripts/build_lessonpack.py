...
SECTION_BODIES = {
    ...
    "sparsa-varga": {
        ...
        "prodolzhayushchiy": (
            "Прежде чем читать классификацию, проверьте её на себе: скажите «к», «т», "
            "«п» и заметьте общее — в каждом случае воздух на мгновение полностью "
            "перекрывается (языком или губами), это и называется смычка. Группа "
            "**sparśa** («прикосновение») — как раз все смычные согласные плюс их "
            "носовые пары (там, где воздух вместо рта идёт в нос — сравните «т» и «н»); "
            "классификация физиологическая, восходящая к традиционной индийской "
            "фонетике (śikṣā). Деление на пять **varga** — рядов — задаёт "
            "двухпризнаковую матрицу: голос (глухость/звонкость) × придыхание, "
            "замыкаемую носовым.\n\nЭта системность — не учебная условность, а живая "
            "организация варга-таблиц, которую вы будете видеть в любой грамматике "
            "санскрита."
        ),
    },
    "five-rows": {
        ...
        "prodolzhayushchiy": (
            "Проверьте матрицу на себе прежде, чем запоминать названия: «к» → «ч» → "
            "«т-загнутое» → «т» → «п» — язык идёт спереди назад через ровно пять точек. "
            "Усвойте её один раз, дальше она повторяется ряд за рядом:"
        ),
    },
    "pronunciation": {
        "nol": (
...
# Five-rows table, built once and shared by both registers so an interest annotation
# lands on the SAME row regardless of level — the worked example threads through the
# table itself instead of trailing it as an aside (H3824 lever 3).
FIVE_ROWS = [
    {"id": "velar", "label_nol": "Заднеязычные (та самая точка зевка)",
     "label_prod": "Заднеязычные",
     "glyphs": "क ka · ख kha · ग ga · घ gha · ङ ṅa"},
    {"id": "palatal", "label_nol": "Палатальные (чуть впереди)",
     "label_prod": "Палатальные",
     "glyphs": "च ca · छ cha · ज ja · झ jha · ञ ña"},
    {"id": "cerebral", "label_nol": "Церебральные, или ретрофлексные (язык загнут назад — «ложка»)",
     "label_prod": "Церебральные (ретрофлексные)",
     "glyphs": "ट ṭa · ठ ṭha · ड ḍa · ढ ḍha · ण ṇa"},
    {"id": "dental", "label_nol": "Зубные (у верхних зубов, как русское т/д)",
     "label_prod": "Зубные",
     "glyphs": "त ta · थ tha · द da · ध dha · न na"},
    {"id": "labial", "label_nol": "Губные (губами)", "label_prod": "Губные",
     "glyphs": "प pa · फ pha · ब ba · भ bha · म ma"},
]

FIVE_ROWS_CLOSING = {
    "nol": "Заметьте: порядок внутри ряда всегда один и тот же — глухой, глухой "
           "придыхательный, звонкий, звонкий придыхательный, носовой.",
    "prodolzhayushchiy": "Церебральный ряд — главная типологическая примета "
        "санскритского звучания для носителя русского: контраст т/т-ретрофлексный "
        "(рус. *т* vs хинди ट) здесь фонематичен.",
}

# Interest annotations attach INLINE to the row(s) their lesson word's consonant
# actually belongs to — not a paragraph appended after the whole table (H3824 lever
# 3: the interest becomes part of the worked example, not a footnote).
ROW_INTEREST_NOTES = {
    "yoga": {
        "velar": "→ слово урока **gaṇa** («множество») тоже начинается с этого ряда "
                 "(ग) — вы могли слышать его в имени *Gaṇeśa*, «господин свиты».",
    },
    "ayurveda": {
        "dental": "→ **dhana** («богатство») — из этого ряда (ध); аюрведа считает "
                  "здоровье главным богатством.",
        "labial": "→ **bhaga** («счастье») — из этого ряда (भ).",
    },
    "kino": {
        "cerebral": "→ **naṭaka** («актёр, танцор») — из этого ряда (ट/ण); индийское "
                    "кино выросло из naṭaka-традиции.",
    },
    "palomnichestvo": {
        "labial": "→ **patha** («путь») начинается с этого ряда (п) — маршрут "
                  "паломника к храму записывали именно devanāgarī.",
    },
}

# Remaining swaps — smaller sections where a trailing aside still reads naturally.
SWAPS = {
    "yoga": [
        {"after": "writing-rules",
...
def render_personalized_text(level, interest, fixture, quizzes, emb, ctx) -> str:
    ...
    for sec in SECTIONS:
        ...
        parts.append("")
        if sec["id"] == "five-rows":
            notes = ROW_INTEREST_NOTES.get(interest, {}) if interest != "base" else {}
            for i, row in enumerate(FIVE_ROWS, start=1):
                label = row["label_nol"] if reg == "nol" else row["label_prod"]
                parts.append(f"{i}. **{label}**: {row['glyphs']}")
                note = notes.get(row["id"])
                if note:
                    parts.append(f"   {note}")
            parts.append("")
            parts.append(FIVE_ROWS_CLOSING[reg])
            parts.append("")
        if interest != "base":
