...
def render_book(work, entries, book_dir, synthesis="", harvest=None):
    ...
    # Methodichka «Учащемуся» takeaways — the single source of the per-claim learner advice
    # that the print methodichka (METODICHKA_*_KOMMENTARII_2026.md) and the <ApteClaims/> reading-
    # site overlay both render, so print and web can never diverge (H1095).
    methodichka = [(e.get("id"), e.get("methodichka")) for e in entries if e.get("methodichka")]
    if methodichka:
        lines += ["", "## Методичка — учащемуся",
                  "", "_Per-claim learner takeaways, mirrored verbatim into the "
                  "`<ApteClaims/>` reading-site overlay via `claims.json` — edit them here, "
                  "never in the component (H1095)._", ""]
        for cid, m in methodichka:
            lines.append(f"- **{md_cell(cid)}** — {md_cell(m)}")
