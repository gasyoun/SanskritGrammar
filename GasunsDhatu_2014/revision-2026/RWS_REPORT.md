# RWS-проход «Совет филологов» по прозе диссертации — отчет для издательского редактора

_Created: 07-07-2026 · Last updated: 09-07-2026_

Отчет по решению MG Q6 (06-07-2026): агентная стилистическая критика прозы книги
«Состав и строй древнеиндийских корней» перед передачей человеку-издательскому
редактору. Отвечает на замечание C1 трех рецензентов защиты 2014 г. (Серебряный:
...
- **Совет:** именованный совет `sanskrit` из 6 стилей-рецензентов:
  `elizarenkova-veda`, `toporov-etym`, `panini-traditional`, `zalizniak-method`,
...
- **Модели:** рецензии — DeepSeek `deepseek-chat` (на 07-07-2026 сервером резолвится в
  ...
  `deepseek-chat` (7 из 28 советов). Оркестрация, агрегирование и курирование отчета —
  Fable 5 (`claude-fable-5`).
- **Объем:** основной текст [02_gasuns-dhatu-PhD-text2.mdx](https://github.com/gasyoun/SanskritGrammar/blob/main/GasunsDhatu_2014/02_gasuns-dhatu-PhD-text2.mdx)
...
- **Ранжирование:** балл абзаца = Σ веса серьезности замечаний (critical 3 / major 2 /
...
**Рецепт для редактора:** для каждого такого места одно из трех — (а) ссылка с страницей,
...
«звездочек»-реконструкций, тут же названных «интересным материалом» (№ 4, Введение);
в статье 3 проза 2014 г. противоречит собственным таблицам (№ 17 — частично уже снято
пересчетом Табл. 5 в [errata.yml](https://github.com/gasyoun/SanskritGrammar/blob/main/GasunsDhatu_2014/errata.yml),
...
термин дается в русской передаче + IAST в скобках, далее свободно; (б) один
...
полноты» (№ 8), тезис о неизменности методологии со ссылкой на Мейе без учета
...
§1 — Заключение должно собираться из них, а не параллельно им; фактическую базу дает
...
всякой связки следующий абзац начинает «Помимо Р. К. Раска… известен еще по крайней
...
лет до нее…» + явный подзаголовок; (б) композиционный — перенести блок Вестергаард–Бетлинк
...
Формат: раздел → место в файле (кликабельно) → фрагмент → диагнозы (до трех самых
...
- **zalizniak-method** (major): Фраза 'корни, у которых не хватало форм или совпадали определенные формы, или были известны только именные деривативы и не известны финитные формы' не различает дефектность в синхронии (у Панини) и диахронии (историческая утрата). Требуется логическая ясность.
  - *Правка:* Разделить дефектность на: (а) парадигматические лакуны в системе Панини (например, корни, не образующие всех времен) и (б) историческую утрату финитных форм, засвидетельствованную в ведийском корпусе.
...
- _…и еще 11 замечаний(я) от 6 рецензентов — см. приложение._
...
- **zalizniak-method** (major): Утверждение, что Панини должен был предположить наличие шипящего в корнях *majj* и *bhrajj*, не мотивировано: неясно, идет ли речь о реконструкции праформы или о графической репрезентации. Отсутствует ссылка на конкретное правило Аштадхьяи.
...
- _…и еще 6 замечаний(я) от 6 рецензентов — см. приложение._
...
- **toporov-etym** (major): Фраза 'здесь почти отсутствуют столь привычные в работах по компаративистике «звездочки»' указывает на намеренный отказ от реконструированных форм, что противоречит основному требованию стиля: обязательно отделять реконструкцию от засвидетельствованного материала. Стиль предписывает использовать звездочку для праформ.
  - *Правка:* Добавить реконструированные формы со звездочкой (*) и различать их с засвидетельствованными. Например, если упоминается 'никогда не существовавшие формы', следует привести конкретные примеры с реконструкцией.
...
- **tronsky-readings** (major): Утверждается, что работа 'избегает лингвистической внутренней и внешней реконструкции' и 'стремится к порождающей грамматике', но не приводится ни одного примера такой порождающей грамматики или конкретного правила. Это голословное заявление без опоры на материал.
  - *Правка:* Привести хотя бы один пример порождающего правила или сослаться на соответствующее исследование. Иначе снять это утверждение как неподкрепленное.
- _…и еще 4 замечаний(я) от 5 рецензентов — см. приложение._
...
- **panini-traditional** (major): Утверждение 'вряд ли вообще существуют объективные критерии классификации корпуса корней' является сильным заявлением, не подкрепленным ссылкой на научный источник или обсуждение в традиции. Требуется опора на литературу (например, Whitney, Renou, Paribok).
...
- **zalizniak-method** (major): Утверждение 'вряд ли вообще существуют объективные критерии классификации корпуса корней' голословно и противоречит методу Зализняка, требующему строгих критериев. Не предложено альтернативной процедуры.
...
- _…и еще 5 замечаний(я) от 5 рецензентов — см. приложение._
...
- **panini-traditional** (major): Термин «корень» используется без различия между индийской (dhātu) и западной концепцией; не введен и не объяснен туземный термин в IAST.
...
- **zalizniak-method** (major): Фраза «внутренне логичную систему научных воззрений, представляющую собой важное достижение лингвистической мысли» не подкреплена ссылками на конкретные работы или традиции. Это оценочное суждение без доказательств.
...
- _…и еще 4 замечаний(я) от 6 рецензентов — см. приложение._
...
- **elizarenkova-veda** (major): Утверждение «акцентная характеристика ... в древнеиндийском привязана к корню» — упрощение. В ведийском акцент подвижен и не всегда закреплен за корнем (например, сдвиг в префиксированных формах). Это вводит в заблуждение относительно ведийской акцентологии.
...
- **zalizniak-method** (major): Утверждается, что "акцентная характеристика в древнеиндийском привязана к корню" без доказательств и ссылок на акцентологические исследования. Это упрощение, противоречащее данным о подвижности акцента в санскрите.
...
- _…и еще 3 замечаний(я) от 5 рецензентов — см. приложение._
...
- **zalizniak-method** (major): Автор вводит запись √vAsH для корня *vas* как seṭ, но в оригинальной системе Зализняка (1975) заглавные буквы используются для обозначения неизменяемости гласного, а не seṭ-ности. Здесь 'A' и 'H' трактуются произвольно, что противоречит аккуратности записи Зализняка.
...
- **lidova-commentary** (major): Предложение использовать морфонологическую запись Зализняка с символом H не сопровождается объяснением, как эта запись соотносится с традицией Панини и последующей комментаторской школой. Не указано, опирается ли автор на какие-либо комментарии к дхатупатхе или на работы по истории грамматики.
  - *Правка:* Добавить контекст: например, отметить, что обозначение seṭ-ности через условный знак H восходит к идее 'абстрактного i' в трудах Ф. Б. Я. Кейпера или к дискуссиям о статусе i-вставки в школьной традиции. Указать, какие именно комментаторские источники (например, 'Кашика-вритти' или 'Сиданта-каумуди') подтверждают или опровергают такое обозначение.
- **zalizniak-method** (minor): Упоминается 'по дополнительной таблице «Распределение корней по признаку *aniṭ*-*seṭ*» [Зализняк 1975, 83]', но такая таблица отсутствует в указанном источнике. Возможно, имеется в виду другая таблица или страница.
  - *Правка:* Проверить ссылку и указать точное название таблицы или исправить номер страницы.
- _…и еще 4 замечаний(я) от 4 рецензентов — см. приложение._
...
- **zalizniak-method** (major): Фраза «находится вне всяких сомнений» создает иллюзию абсолютной достоверности, что не соответствует стилю Зализняка, требующему доказательности, а не риторической уверенности. Кроме того, утверждение о том, что «древнеиндийский язык содержит достаточно верное отражение древней глагольной системы», требует оговорки об известных инновациях (например, презентные основы на -ya-).
...
- **elizarenkova-veda** (major): Система *анубандх* названа «своеобразной и удобной», что является субъективной оценкой без анализа конкретных примеров. Стиль требует опоры на форму и контекст, а не голословных утверждений.
  - *Правка:* Заменить на: «...система анубандх (*anubandha*), применяемая в индийской грамматической традиции, используется для маркирования грамматических характеристик корней, однако ее эффективность может быть оценена по-разному (см. Захарьин 2007)».
- **lidova-commentary** (major): Упоминание анубандх без объяснения их функции в традиции Панини; не показано, почему система пометок Рикса кажется сомнительной по сравнению с ними.
  - *Правка:* Добавить: 'В системе Панини анубандхи выполняют роль метаязыковых маркеров, экономно кодирующих информацию; аппарат пометок в LIV лишен такой системности и потому менее функционален.'
- **panini-traditional** (major): Упоминание 'анубандх' (anubandha) как 'системы языковых маркеров' дается в скобках со ссылкой на 'Захарьин 2007', но не атрибутируется конкретному комментатору (например, Пандиту) и не излагается через сутра-вритти. Стиль требует 'толкование конкретному комментатору'.
...
- _…и еще 2 замечаний(я) от 5 рецензентов — см. приложение._
...
- **elizarenkova-veda** (major): Фраза «многие корни санскрита встречаются в лексемах нескольких склонений/спряжений» не различает ведийский и классический санскрит. Для ведийского языка корни и их формы специфичны; требуется уточнение, что речь идет о ведийском санскрите, а не о более позднем.
...
- **zalizniak-method** (major): Список вариантов корня √hū/hve/hvā не содержит указаний на конкретные страницы или параграфы из Burnouf, Coulson, Gonda, Perry, Whitney и других. Отсутствие точных ссылок противоречит требованию доказательности.
...
- **tronsky-readings** (major): Предложение 'Стоит также задуматься о новом полном санскритско-русском словаре' не подкреплено анализом существующих словарей (Бехтлинк, Монье-Вильямс, Кочергина) и их недостатков. Стиль Tronsky-Readings требует строгой опоры на предшествующую литературу.
...
- _…и еще 3 замечаний(я) от 4 рецензентов — см. приложение._
...
- **panini-traditional** (major): Термин 'dhātu' переведен как 'глагольный корень' без предварительного введения и разъяснения в духе традиции. Стиль требует вводить важные термины в русской передаче и IAST, например 'дхату (dhātu)', и лишь затем соотносить с европейским 'глагольный корень'. Здесь же термины 'bhūvādayo dhātavaḥ' даны в IAST, но 'dhātu' не выделен как технический термин.
...
- **zalizniak-method** (minor): Фраза «некоторые авторитетные ученые утверждают, что их было меньше» не сопровождается конкретными ссылками на работы этих ученых (кроме Cardona). Для стиля Зализняка недопустимо ссылаться на «некоторых» без указания имен и публикаций.
...
- **zalizniak-method** (major): Не указано, на основании каких критериев автор судит о 'научности' и 'реальности' значений корней в дхатупатхе.
...
- **toporov-etym** (minor): Значения корней в дхатупатхе названы 'ненаучными', но не приведено ни одного конкретного примера такой ненаучности, что снижает обоснованность утверждения.
  - *Правка:* Привести 1–2 примера корней с 'ненаучным' значением и пояснить, в чем состоит ненаучность.
- _…и еще 2 замечаний(я) от 5 рецензентов — см. приложение._
...
- **tronsky-readings** (major): Перечисление возможных статистических подсчетов дано без указания, на основании какой методики и корпуса текстов они предполагаются. Отсутствует аппарат (ссылки на программы, методики, предшествующие аналогичные исследования).
...
- **zalizniak-method** (major): Предлагаемые статистические подсчеты (частотность слогов, консонантных кластеров и т.д.) не обоснованы ссылками на существующие корпусные исследования или методологию.
  - *Правка:* Указать, на каком корпусе (например, Rigveda, Mahabharata) и с помощью каких инструментов (программы, алгоритмы) предполагается проводить подсчеты, или сослаться на существующие работы (например, Edgerton 1943).
...
- **zalizniak-method** (major): Автор говорит 'в схеме Зализняка veṭ приходилось обозначать словами (то есть использование специальных знаков не предусмотрено)', однако у Зализняка (1975: 70) veṭ указывается с помощью пометы 'V' или словесно, но это не 'не предусмотрено', а сознательное решение. Утверждение неточно.
...
- _…и еще 2 замечаний(я) от 4 рецензентов — см. приложение._
...
- **toporov-etym** (minor): Утверждение: 'термин dhātu образован от корня dhā- «ставить, класть»'. Это общепринятая этимология, но не указано, что она основана на регулярных соответствиях. Стиль требует разграничения надежного и гипотетического.
...
- **toporov-etym** (major): Утверждение о том, что корни, не проходящие по определенным критериям, называются «нелегальными» или «неправильными», не подкреплено ссылкой на источник и не объяснено с точки зрения регулярных соответствий. Такая терминология субъективна и не соответствует научному стилю.
  - *Правка:* Использовать нейтральную терминологию (например, «исключенные из списка») и указать, на основании каких критериев (длина слога, наличие двух гласных и т.д.) корни были отброшены, со ссылкой на Пальсуле или другой источник.
- **zalizniak-method** (major): Фраза 'не дифференцируется и в списке «нелегальных»… одни и те же корни могу иметь два или даже все три признака непригодности' логически неполна: неясно, о какой дифференциации идет речь, и какова процедура отбора. Зализняк требует абсолютной логической связности, а здесь допущена неоднозначность.
...
- _…и еще 1 замечаний(я) от 4 рецензентов — см. приложение._
...
> Пальсуле во многом опирается на грамматику Панини и приложения к ней в редакции Бетлинка, об этом косвенно свидетельствуют сноски к отдельным глоссам. Но Бетлинковское издание -- это переработка списка Вестергаарда (на п…

- **panini-traditional** (major): Фраза 'Пальсуле во многом опирается на грамматику Панини и приложения к ней в редакции Бетлинка' не содержит прямой ссылки на конкретное место из Пальсуле, где это утверждение обосновано. Хотя далее упоминаются косвенные свидетельства (сноски), это не является достаточной атрибуцией. Согласно стилю, толкование должно быть приписано конкретному комментатору (Пальсуле) и подкреплено указанием на источник.
  - *Правка:* Добавить указание на конкретные страницы или параграфы работы Пальсуле (Palsule 1955), где обсуждается его опора на Бетлинка. Например: 'Пальсуле (Palsule 1955, с. x) прямо указывает, что его конкорданс базируется на издании Бетлинка.'
- **panini-traditional** (major): Термин 'дхату' (dhātu) употреблен в фразе 'что же имеется в виду под термином дхату' без разъяснения, что в панинеевской традиции dhātu означает не просто 'корень', а глагольный корень как элемент грамматической системы. В контексте перевода Бетлинка ('Würzeln in Sinne von Pāṇini') смешение может ввести в заблуждение. Стиль требует вводить туземный термин с пояснением и IAST.
...
- **lidova-commentary** (major): Упоминание «Бетлинк допускает, что в течении времени кое-что возможно и вкралось в список» без указания на то, как этот тезис связан с более широкой проблемой авторитета и изменчивости комментаторской традиции. Стиль Лидовой требует помещать такое наблюдение в рамку традиции: кто передает текст, как меняется его функция при переходе от устной к письменной фиксации.
  - *Правка:* После цитаты Бетлинка добавить связку: «Это замечание характерно для западных комментаторов XIX в., которые, с одной стороны, признавали авторитет Панини, а с другой — стремились критически пересмотреть состав корней, что отражает напряженность между уважением к традиции и требованиями филологической точности.»
- _…и еще 2 замечаний(я) от 4 рецензентов — см. приложение._
...
> Бетлинк отдает себе отчет, что из-за объединения нескольких источников в единый текст приложений попали «испорченные и неправильные словоформы» (verdorbene und falsche Wortformen) [Boethlingk 1887, 301\*]. Но риск искале…
...
- **lidova-commentary** (major): Перечислены Бетлинк, Вестергаард, Либих, Пальсуле как редакторы, не добавлявшие новых корней из пиетета, но не раскрыта функция такого консерватизма в традиции. Стиль Лидовой требует различать технический (сохранение материала) и аналитический (осмысление изменчивости) уровни комментария.
  - *Правка:* Добавить анализ: «Такая осторожность показывает, что редакторы видели свою задачу прежде всего в техническом сохранении материала, а не в аналитической критике. Однако тем самым они закрепляли неопределенность состава дхатупатх, что только усиливало потребность в дальнейших комментариях.»
- **panini-traditional** (major): В сегменте p008 идет речь о 'списке Панини' (dhātupāṭha), но ни разу не упоминается конкретная сутра, на основании которой корень считается принадлежащим или не принадлежащим Панини. Например, Пальсуле отвергает корень *2aṭ*, но не указано, на какую сутру при этом ссылаются. Это нарушает требование стиля о том, что сутра должна быть указана с номером.
...
## 6. Ограничения метода и как пользоваться отчетом

- **Рецензенты — LLM-персоны** (deepseek-v4-flash), а не филологи. Диагнозы «здесь нет
  ссылки / здесь самоопровержение / здесь разговорный регистр» надежны и проверяемы по
...
| 111 | Обзор литературы | [02_gasuns-dhatu-PhD-…:753](https://github.com/gasyoun/SanskritGrammar/blob/main/GasunsDhatu_2014/02_gasuns-dhatu-PhD-text2.mdx#L753) | 7 | 3 | 3 | `anachronistic_sanskrit_period`, `unsupported_reading` | Возьмем, например, для рассмотрения явление отыменных образований -- они сравнит… |
...
| 130 | Обзор литературы | [02_gasuns-dhatu-PhD-…:437](https://github.com/gasyoun/SanskritGrammar/blob/main/GasunsDhatu_2014/02_gasuns-dhatu-PhD-text2.mdx#L437) | 6 | 2 | 3 | `unsupported_semantization`, `arbitrary_sound_change` | И Отто фон Бетлинк, и Георг Бюлер [Bühler 1972] в свое время обратили пристально… |

---

## 6. Издательские решения по проходу — визы автора (09-07-2026)

Решения автора по §3–§5 настоящего отчета, зафиксированные как визы. По конвенции
...
Принят **вариант (а)**: при первом упоминании термина в каждой главе дается русская
...
- **`elizarenkova-veda` (привязка к гимну)** — **ОТКЛОНЕНО как неприменимое.** Работа не
  ведется с гимнами Ригведы; привязать грамматическое правило к конкретному гимну по
...
Принят **вариант (б)**: перенести блок Вестергаард–Бетлинк ПЕРЕД полемику Уитни–Бюлера,
восстановив хронологию обзора (1841 → 1885); соседние абзацы № 31, 33, 34 приложения А —
в том же переносе. Правка хирургическая (блок перекрестно связан оборотами «как уже
упоминалось» / «но вернемся к более современным…»), поэтому исполняется отдельной
...
Две поправки автора к исходному предложению отчета:

1. **Пальсуле** отнесен к **итогу индийской традиции (дхатупатхи)**, а не к европейской науке.
...
### 6.5. §5 № 1 — разграничение дефектности (`zalizniak-method`): ПРИНЯТО целиком

Принята правка — разделить дефектность корней на **(а)** парадигматические лакуны в
системе Панини (корни, не образующие всех времен) и **(б)** историческую утрату финитных
...
`zalizniak-method`, но и остальными ипостасями Зализняка из Совета. В
[RuWritingStyles](https://github.com/gasyoun/RuWritingStyles) их **девять** паспортов:
`zalizniak-method`, `zalizniak-novgorod`, `zalizniak-enklitiki`, `zalizniak-imennoe`,
`zalizniak-ocherk`, `zalizniak-shkolnikov-1`, `zalizniak-slovo`, `zalizniak-udarenie`,
`zalizniak-zametki`. В совете `sanskrit` участвовал только `zalizniak-method`; остальные
...
ее текст/перевод, чтобы правило в тексте могло цитировать точный адрес. См. H413.
...
`{data:[{sutra,vartika}]}` в vartika.txt отличается от 5-значного id остальных слоев — в H413
...
негитуемый); агрегат продублирован рядом с отчетом в
[RWS_FINDINGS.tsv](https://github.com/gasyoun/SanskritGrammar/blob/main/GasunsDhatu_2014/revision-2026/RWS_FINDINGS.tsv).

_Отчет сгенерирован конвейером RuWritingStyles и откурирован Fable 5 (`claude-fable-5`), H275._
