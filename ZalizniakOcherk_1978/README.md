...
— **74 проверенных утверждения: 57 TRUE · 0 OVERSTATED · 0 FALSE · 17 UNTESTABLE ·
7 частотных сносок М.Г.**;
...
3. **17 UNTESTABLE — это список требований к следующим инструментам конвейера,** а не
...
python ZalizniakOcherk_1978/verify_claims_dcs.py   # вся батарея книги (census, i-rates, §68 join)
```

### Что дальше

Инструменты из списка UNTESTABLE (периодная разметка → §207; кроссволк рядов → §68;
treebank → §§214-217); реестр Конспекта-2004 с откалиброванной планкой; печатный лист
опечаток для `errata.yml`; закрытие `@DECIDE` по окну якоря N=8 квантификаторного слоя.
